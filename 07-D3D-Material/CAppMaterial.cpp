...
CAppMaterial::CAppMaterial() {
}
CAppMaterial::~CAppMaterial() {
}
void CAppMaterial::OnInit() {
	//camera
...
void CAppMaterial::OnUpdate(DWORD fDeltaTime) {
...
void CAppMaterial::OnRender(DWORD fDeltaTime) {
	//light
	D3DLIGHT9 light;
	...
	pD3DFramework->pD3DDevice->SetStreamSource(
		...
		pVB, //버텍스버퍼
		0, //메모리시작 위치
		sizeof(SVertex3) //버텍스 크기
	);
	//2. D3D에 정점 데이터를 어떻게 해석해하는지 설정한다.
	pD3DFramework->pD3DDevice->SetFVF(D3DFVF_SVertex3);
	pD3DFramework->pD3DDevice->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 2);
}
void CAppMaterial::OnRelease() {
	if (pVB) {
		pVB->Release();
	}
}